...
namespace BOOSEprogram1
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            btnRun.Click += (s, e) => { RunScript(txtProgram.Text); Invalidate(); };
            Controls.Add(btnRun);
        ...
        private void RunScript(string src)
        {
            ...
            foreach (var raw in lines)
            {
                ...
                switch (cmd)
                {
                    case "moveto": { var (x, y) = Two(args); canvas.MoveTo(x, y); break; }
                    case "drawto": { var (x, y) = Two(args); canvas.DrawTo(x, y); break; }
                    case "pen":
                    case "setcolour": { var (r, g, b) = Three(args); canvas.SetColour(r, g, b); break; }
                    case "circle": { int r = One(args); canvas.Circle(r, false); break; }
                    case "rect": { var (w, h) = Two(args); canvas.Rect(w, h, false); break; }
                    case "tri": { var (w, h) = Two(args); canvas.Tri(w, h); break; }
                    case "write": { canvas.WriteText(args.Trim().Trim('"')); break; }
                    case "clear": canvas.Clear(); break;
                    case "reset": canvas.Reset(); break;
                }
            }
        }
        ...
        {
            var p = s.Split(',', StringSplitOptions.RemoveEmptyEntries).Select(t => t.Trim()).ToArray();
